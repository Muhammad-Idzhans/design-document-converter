    ...
    try:
        ...
        try:
            for i, slide in enumerate(presentation.Slides, 1):
                export_path = str(output_dir / f"slide_{i}.jpg")
...
def download_pdf(task_id: str, filename: str = "Enfrasys_Design_Document"):
    ...
    try:
        ...
        word.Visible = False
        doc = word.Documents.Open(str(docx_path.resolve()), ReadOnly=True, Visible=False)
...
def prepare_preview(task_id: str):
    ...
    if last_error is not None:
        raise HTTPException(
            status_code=500,
            detail=f"Word COM conversion failed: {str(last_error)}"
        )
