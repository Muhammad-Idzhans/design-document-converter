...
# UPLOAD_DIR = SCRIPT_DIR / "api_uploads"
...
#             print(f"[\u2714 SUCCESS] FastAPI connected to Microsoft Foundry!")
#             print(f"             Bound to Agent: {ans_name} (v{ans_ver})")
#             print(f"             (Bing Search will trigger automatically on generation)")
#         except Exception as e:
#             print(f"[\u274C WARNING] Failed to connect to Microsoft Foundry!")
#             print(f"             Reason: {str(e)}")
#             print(f"             Fallback: Standard Offline GPT will be used.")
#     else:
#         print(f"[\u2139 INFO] No AGENT_ASSISTANT_ID detected in .env.")
#         print(f"          Fallback: Standard Offline GPT will be used.")
#     print("="*60 + "\n")
...
# TASKS_FILE = SCRIPT_DIR / "tasks_db.json"
# TASKS_LOCK = Lock()
...
#     # Try LibreOffice universally first (Primary for Linux/Docker)
#     try:
#         if os.name == 'posix':
#             subprocess.run(['soffice', '--headless', '--convert-to', 'pdf', '--outdir', os.path.dirname(abs_pdf), abs_pptx], check=True)
#             # Libreoffice names the file after the original, we need to rename it to exactly match pdf_path
...
#     # Windows Native Fallback (Primary for Local Dev)
#     import win32com.client
#     import pythoncom
...
#         presentation.Close()
#     except Exception as e:
#         print(f"Fallback PPTX to PDF conversion failed: {e}")
#     finally:
#         if powerpoint:
#             powerpoint.Quit()
...
#     try:
#         if os.name == 'posix':
#             subprocess.run(['soffice', '--headless', '--convert-to', 'pdf', '--outdir', os.path.dirname(abs_pdf), abs_docx], check=True)
#             expected_out = os.path.join(os.path.dirname(abs_pdf), os.path.splitext(os.path.basename(abs_docx))[0] + ".pdf")
#             if expected_out != abs_pdf and os.path.exists(expected_out):
#                 os.rename(expected_out, abs_pdf)
#             return
#     except Exception:
...
#         doc.Close(False)
#     except Exception as e:
#         print(f"Fallback DOCX to PDF conversion failed: {e}")
#         try:
...
# def generate_slide_thumbnails(pptx_path: str, output_dir: Path):
#     import os, fitz
#     # First, securely convert PPTX to PDF using our cross-platform wrapper
...
#     try:
#         # Cross-platform Lightning Fast PDF-to-Image Extraction via PyMuPDF
#         doc = fitz.open(temp_pdf)
#         for i in range(len(doc)):
#             page = doc.load_page(i)
#             pix = page.get_pixmap(matrix=fitz.Matrix(2, 2))  # High-Resolution Matrix
...
# def extract_preview(pptx_path: str, task_id: str) -> dict:
#     from pptx import Presentation
...
# def extract_with_pptx(pptx_path: str, output_dir: Path) -> dict:
#     prs = Presentation(pptx_path)
...
#         slide_info = {"slide_number": slide_idx, "title": None, "text_content": [], "speaker_notes": None, "images": [], "tables": []}
...
#         return None
#     analyze_binary_url = f"{CONTENT_UNDERSTANDING_ENDPOINT}/contentunderstanding/analyzers/design_document_converter:analyzeBinary?api-version={API_VERSION}"
...
#     analyze_url = f"{CONTENT_UNDERSTANDING_ENDPOINT}/contentunderstanding/analyzers/design_document_converter:analyze?api-version={API_VERSION}"
#     with open(pdf_path, "rb") as f:
#         file_bytes = f.read()
...
#         - CAPTION RULE: You must write a short, concise caption (1 sentence maximum) STRICTLY BELOW the image (e.g., "Figure 1: High-level data workflow architecture.").
#         - DO NOT place any image descriptions above the image. 
#         - DO NOT copy-paste the long 'ai_description' from the JSON into the document. Use that description internally just to understand the context, but keep the actual output caption extremely brief.
...
#         CORRECT EXAMPLE OF AN ENTIRE APPENDIX SECTION:
#         ### Introduction/Prerequisites
#         Azure Virtual Machines: [https://learn.microsoft.com/en-us/azure/virtual-machines/](https://learn.microsoft.com/en-us/azure/virtual-machines/)

#         Azure Monitor: [https://learn.microsoft.com/en-us/azure/azure-monitor/](https://learn.microsoft.com/en-us/azure/azure-monitor/)

#         ### Limits and Boundaries
#         Azure Subscription Limits: [https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/azure-subscription-service-limits](https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/azure-subscription-service-limits)

#         ### Others
#         Azure Compute SLAs: [https://azure.microsoft.com/en-us/support/legal/sla/virtual-machines/](https://azure.microsoft.com/en-us/support/legal/sla/virtual-machines/)
#     """
...
#         # Process markdown-generated images BEFORE we inject custom logos on the first page
...
#                 # Clear all tabs to prevent tab-stops from wrapping long text
#                 para.paragraph_format.tab_stops.clear_all()
#                 # Apply a single right-aligned tab at the edge (6.3 inches margin for typical widths)
#                 para.paragraph_format.tab_stops.add_tab_stop(Inches(6.3), WD_TAB_ALIGNMENT.RIGHT)
                
#                 # We enforce exactly ONE tab across the entire footer paragraph
#                 tab_count = 0
...
#                     # Ensure font is small enough to fit a long title seamlessly
#                     run.font.size = Pt(9)
...
#     file_path = task_dir / f"source_{file.filename}"
#     with open(file_path, "wb") as f:
#         shutil.copyfileobj(file.file, f)
#     logo_path = None
#     if logo:
#         logo_path = task_dir / f"logo_{logo.filename}"
...
#         # ---------------------------------------------------------
#         # 1. DYNAMIC FOOTER CREATION (No Template Dependency)
#         # ---------------------------------------------------------
#         from docx.enum.text import WD_TAB_ALIGNMENT
#         from docx.oxml import OxmlElement
#         from docx.oxml.ns import qn
...
#             # 1. Clear any existing paragraphs/footers coming from the template
#             for p in footer.paragraphs:
#                 p_element = p._element
#                 p_element.getparent().remove(p_element)
                
#             # 2. Create a fresh, empty paragraph for our new footer
#             para = footer.add_paragraph()
            
#             # 3. Set a Right-Aligned tab stop at 6.5 inches (pushes text after '\t' to the right edge)
#             para.paragraph_format.tab_stops.clear_all()
#             para.paragraph_format.tab_stops.add_tab_stop(Inches(6.5), WD_TAB_ALIGNMENT.RIGHT)
            
#             # 4. Add the Left Side: Project Title + a Tab Character (\t)
#             run_left = para.add_run(f"{clean_title} - Planning & Design Document\t")
#             run_left.font.name = 'Times New Roman'
#             run_left.font.size = Pt(9)
            
#             # 5. Add the Right Side: Dynamic Page Number XML
#             run_right = para.add_run()
#             run_right.font.name = 'Times New Roman'
#             run_right.font.size = Pt(9)
            
#             # --- START OF WORD XML MAGIC FOR DYNAMIC PAGE NUMBERS ---
#             fldChar_begin = OxmlElement('w:fldChar')
#             fldChar_begin.set(qn('w:fldCharType'), 'begin')
            
#             instrText = OxmlElement('w:instrText')
#             instrText.set(qn('xml:space'), 'preserve')
#             instrText.text = "PAGE"  # This tells Word to calculate the current page number
            
#             fldChar_end = OxmlElement('w:fldChar')
#             fldChar_end.set(qn('w:fldCharType'), 'end')
            
#             # Append the XML elements to the run
...
#         # ---------------------------------------------------------
#         # 2. GLOBAL FONT ENFORCEMENT, JUSTIFICATION & CAPTIONS
#         # ---------------------------------------------------------
#         from docx.enum.text import WD_ALIGN_PARAGRAPH
...
#         figure_counter = 1  # Global counter to ensure continuous numbering!
...
#             # --- A. FIX IMAGE CAPTIONS (Renumber & Center) ---
#             if text.lower().startswith("figure ") and ":" in text:
#                 # Regex replaces "Figure 1:", "Figure 2 :", etc., with the running counter
...
#                 figure_counter += 1
#                 continue  # Skip the rest of the loop so we don't overwrite caption styling
...
#             if para.style.name == "Heading 1" or any(text.startswith(f"{i}.0") for i in range(2, 21)):
#                 # Protect 1.0 (Sign-off) and 2.0 (Exec Summary) from getting double page breaks!
...
#         # ---------------------------------------------------------
#         # 3. TABLE FIXES (Banding, Widths, and Fonts)
#         # ---------------------------------------------------------
#         from docx.oxml.ns import qn
#         from docx.oxml import OxmlElement
...
#             # Turn off vertical banding via XML
#             tblPr = table._tbl.tblPr
#             tblLook = tblPr.find(qn("w:tblLook"))
#             if tblLook is None:
#                 tblLook = OxmlElement('w:tblLook')
...
#             # Force wide tables to 100% width
#             if len(table.columns) >= 5:
#                 tblW = tblPr.find(qn('w:tblW'))
#                 if tblW is None:
#                     tblW = OxmlElement('w:tblW')
...
#     except Exception as e:
#         # preserve original behavior (silent), but print for Azure logs
